...
async function activity(request, env) {
  ...
  const recent = await cardApi(env, {
    q: 'Pokemon', platform: 'ebay', category: 'tcg', sort: 'date_desc', limit: 50,
  });
  if (!recent.ok) {
    return json({ error: recent.message, upstreamStatus: recent.status, fatal: [401,403,429].includes(recent.status), rate: recent.rate }, recent.status);
  }
  ...
  const results = cards.map(card => {
    ...
    const active = !!bestRow && bestMatch >= 45;
    return {
      key: String(card?.key || ''),
      active,
      match: bestMatch,
